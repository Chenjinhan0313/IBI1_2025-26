...
dalys_data.iloc[0,3]#access values by row and column numbers
...
dalys_data.iloc[0:3,[0,1,3]]#only show the first,second and fourth columns of the first three rows
#another way
...
dalys_data.loc[2:4,'Year']#access adta by row nunmers and column names (that is because row don't have names)
...
print('Comment:the first year these data were recorded:1990, the last year these data were recorded:2019')
boolean2=dalys_data['Year'] == 2019
dalys_data_proccesed=dalys_data.loc[boolean2,:]
description=dalys_data_proccesed['DALYs'].describe()
boolean3=dalys_data_proccesed['DALYs'] == description['max']
print(f'{dalys_data_proccesed.loc[boolean3,:].loc[:,'Entity'].iloc[0]} recorded the largest DALYs values in the most recent year')
boolean4=dalys_data_proccesed['DALYs'] == description['min']
...
plt.xlabel('Year')
plt.ylabel('DALYs (Disability-Adjusted Life Years per 100000)')
